<template>
    <Button @click="openDialog" label="Edit" icon="pi pi-plus" severity="warning" class="mr-2" :disabled="isDisabled" />
    <Dialog v-model:visible="isDialogVisible" :style="{width: '450px'}" header="Edit task" :modal="true">
        ...
        <template #footer>
            ...
            <Button @click="editTask" label="Edit" :loading="adding" />
        </template>
    </Dialog>
    <Toast />
</template>
...
onMounted(() => {
    ...
    initTeamMembers(projectId.value)
})
...
const initTeamMembers = (projectId) => {
    const project = projectStore.getProject(projectId)
    employees.value = project.team.teamMembers
}

const initFormValues= () => {
    setFieldValue('name',props.task.name)
    setFieldValue('description',props.task.description)
    setFieldValue('employee',props.task.employee)
}
...
const editTask = handleSubmit(values => {
    projectStore.editTask(projectId.value, props.task.id, {
        ...values
    }).then(data => {
        toast.add({ severity: 'success', summary: 'Editing the task', detail: data, life: 3000 });
    }).catch(error => {
        toast.add({ severity: 'error', summary: 'Editing the task', detail: error, life: 3000 });
    }).finally(() => {
